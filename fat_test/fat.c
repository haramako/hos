// Copy from
// https://github.com/nuta/resea/blob/f20d1c77725fddef59c36df027ba56ac7ef4705a/servers/fatfs/fat.c
#include "fat.h"
...
error_t fat_probe(struct fat *fs, void (*blk_read)(size_t offset, void *buf, size_t len),
	...
	size_t non_data_sectors = bpb.num_reserved_sectors + (bpb.num_fat * sectors_per_fat) + root_dir_sectors;
	size_t total_data_clus = (sectors - non_data_sectors) / bpb.sectors_per_cluster;
...
static bool is_free_cluster(cluster_t cluster) { return cluster == 0; }

static bool is_valid_cluster(struct fat *fs, cluster_t cluster) {
	return !is_end_of_cluster(fs, cluster) && !is_free_cluster(cluster);
}

static bool filename_equals(struct fat_dirent *e, const char *name, const char *ext) {
	int name_len = 8;
	...
	return !strncmp((const char *)e->name, name, name_len) && !strncmp((const char *)e->ext, ext, ext_len);
}
...
error_t fat_read(struct fat *fs, struct fat_file *file, offset_t off, void *buf, size_t len) {
...
void fat_closedir(struct fat *fs, struct fat_dir *dir) { free(dir->entries); }
...
struct fat_dirent *fat_readdir(struct fat *fs, struct fat_dir *dir) {
	...
	dir->index++;
	int num_entries = (fs->sectors_per_cluster * SECTOR_SIZE) / sizeof(struct fat_dirent);
