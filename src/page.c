...
static void print_merged_pme2_(uint64_t base_addr, int merge_start, int merge_end, uint64_t merge_flags) {
	...
	klog("L2: %018p~%018p %10s %3d %s", start_addr, end_addr - 1, humanize_size(end_addr - start_addr), merge_size,
		 pme_flags(f));
}
...
static void print_pml2_(PMEDisplayConfig *conf, PageMapEntry *pml2, uint64_t base_addr) {
	...
	if (merge_start != -1) {
		// Finish display merging.
		print_merged_pme2_(base_addr, merge_start, PAGE_MAP_TABLE_LEN, merge_flags);
	}
}
...
static void print_merged_pme1_(uint64_t base_addr, int merge_start, int merge_end, uint64_t merge_flags) {
	...
	int merge_size = merge_end - merge_start;
	PageMapEntry f = {.raw = merge_flags};
	klog("L1: %018p~%018p %10s %3d %s", start_addr, end_addr - 1, humanize_size(end_addr - start_addr), merge_size,
		 pme_flags(f));
}
...
static void print_pml1_(PMEDisplayConfig *conf, PageMapEntry *pml1, uint64_t base_addr) {
	...
	if (merge_start != -1) {
		// Finish display merging.
		print_merged_pme1_(base_addr, merge_start, PAGE_MAP_TABLE_LEN, merge_flags);
	}
}

void page_map_entry_print(PageMapEntry *pml4) {
	PMEDisplayConfig conf = {.mask = ~0x60};
